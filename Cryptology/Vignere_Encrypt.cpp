...
string vigenereEncrypt(const string &plaintext, const string &key)
{
    ...
    {
        ciphertext += (cleanedText[i] - 'a' + key[i % key.length()] - 'a') % 26 + 'a';
    }
...
int main()
{
    ...
    ofstream outFile("0224.txt");
    if (outFile.is_open())
    {
        outFile << "Cipher Text 1:\n"
                << formattedCipherText1 << "\n"
                << "Cipher Text 2:\n"
                << formattedCipherText2;
        outFile.close();
