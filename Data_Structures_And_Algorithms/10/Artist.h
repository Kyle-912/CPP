...
public:
    Artist();
    Artist(std::string& ID, std::string& name, int followerCount, int popularityScore, std::vector<std::string>& genres);
    const std::vector<std::string> &getArtistGenres() const;
    const std::string &getArtistName() const;
    const int &getArtistFollowers() const;
    const int &getArtistPopularity() const;
};
